...
class Overtime(models.Model):
    ...
    employee_type = fields.Selection(string='Employee type', selection=[('hq', 'HQ Staff'), ('site', 'Site Staff')],required=True, related = "employee_id.employee_type")


    @api.depends("hours_holiday")
    def comp_holiday_hours(self):
        for rec in self:
            if rec.employee_type == 'hq':
                rec.calculated_holiday_hours = rec.hours_holiday * rec.company_id.hours_holiday_rate
            else:
                rec.calculated_holiday_hours = rec.hours_holiday * rec.company_id.hours_holiday_rate_site
    ...
    def comp_weekend_hours(self):
        for rec in self:
            if rec.employee_type == 'hq':
                ...
                rec.calculated_weekend_hours = rec.hours_weekend * rec.company_id.hours_weekend_rate_site
    ...
    def comp_normal_hours(self):
        for rec in self:
            if rec.employee_type == 'hq':
                ...
                rec.calculated_day_hours = rec.hours_normal * rec.company_id.hours_normal_rate_site
    ...
    def comp_night_hours(self):
        for rec in self:
            if rec.employee_type == 'hq':
                ...
                rec.calculated_night_hours = rec.hours_night * rec.company_id.hours_night_rate_site
        ...
        for rec in self:
            ...
            if hours_per_day:
                ...
                # raise UserError(hours_per_day)
                 ...
                 # raise UserError(hours_per_day)
                    normal_salary_site = self.basic_salary if self.company_id.normal_rate_salary_site == 'basic' else self.gross_salary
                    night_salary_site = self.basic_salary if self.company_id.night_rate_salary_site == 'basic' else self.gross_salary
                    weekend_salary_site = self.basic_salary if self.company_id.weekend_rate_salary_site == 'basic' else self.gross_salary
                    holiday_salary_site = self.basic_salary if self.company_id.holiday_rate_salary_site == 'basic' else self.gross_salary
                    # hours_per_day = self.hours_per_day

                    self.overtime_normal = ((normal_salary_site / 30 / hours_per_day) * self.calculated_day_hours)
                    self.overtime_night = ((night_salary_site / 30 / hours_per_day) * self.calculated_night_hours)
                    self.overtime_weekend = ((weekend_salary_site / 30 / hours_per_day) * self.calculated_weekend_hours)
                    self.overtime_holiday = ((holiday_salary_site / 30 / hours_per_day) * self.calculated_holiday_hours)
                    self.overtime = self.overtime_normal + self.overtime_night + self.overtime_weekend + self.overtime_holiday
            else:
                raise ValidationError('Please Configer Your Company Working Days')       
    ...
    hours_normal_rate_site=fields.Float(string="Day Hours Rate",related='company_id.hours_normal_rate_site')
    hours_holiday_rate_site=fields.Float(string="Holiday Rate",related='company_id.hours_holiday_rate_site')
    hours_night_rate_site=fields.Float(string="Night Hours Rate ",related='company_id.hours_night_rate_site')
    hours_weekend_rate_site=fields.Float(string="Weekend Hours Rate",related='company_id.hours_weekend_rate_site')
