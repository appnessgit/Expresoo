...
class BroadFactor(models.Model):
    ...
    emp_broad_factor = fields.Boolean(string="Broad Factor", default=True , help="If check it will display in broad factor type")
...
class Employee(models.Model):
    ...
    def get_user_employee_details(self):
        ...
        leaves_to_approve = self.env['hr.leave'].sudo().search_count([('state', 'in', ['confirm', 'validate1'])])
        appraisals = self.env['hr.appraisal'].sudo().search_count([('employee_id','=', employee[0]['id'])])
        loans = self.env['hr.loan'].sudo().search_count([('employee_id','=', employee[0]['id']),('state','in',['to_approve','approve'])])
        overtime = self.env['hr.over.time'].sudo().search_count([('employee_id','=',employee[0]['id']),('date','>=',first_day_of_month)])
        ...
        if employee:
            ...
            if employee:
                ...
                print("empoooooooooooooooooooooodata",data)
                employee[0].update(data)
            print("empoooooooooooooooooooooo",employee)
            return employee
        else:
            return False
