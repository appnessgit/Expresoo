...
class hr_loan(models.Model):
	...
	def _get_old_loan(self):
		old_amount = 0.00
		for loan in self.search([('employee_id', '=', self.employee_id.id),('state', '!=', 'refuse'),('state', '!=', 'draft')]):
			if loan.id != self.id:
	...
	def _onchange_employee_id(self):
		for rec in self:
			if rec.sudo().employee_id.contract_id:
				rec.emp_salary = rec.sudo().employee_id.contract_id.payroll_wage
	...
	def action_submit(self):
		for rec in self:
			if rec.loan_old_amount > 0:
				raise UserError(_("You Can Not Request New Loan Untill You Pay The Old Loan"))
			else:
				hr_officer_user = []
				hr_officer_user = rec.env.ref('hr.group_hr_user').users
				for officer_user in hr_officer_user:
					rec.activity_unlink(['hr_loan_base.mail_loan_request'])
					rec.activity_schedule('hr_loan_base.mail_loan_request', user_id=officer_user.id)
	...
	def onchange_employee_id(self, employee_id=False):
		old_amount = 0.00
		if employee_id:
			for loan in self.search([('employee_id', '=', employee_id),('state', '!=', 'refuse'),('state', '!=', 'draft')]):
				if loan.id != self.id:
					old_amount += loan.balance_amount
			return {
				'value': {
